...
int main(int argc, const char* argv[]) {
    ...
    id<MTLFunction> mad_throughput = [library newFunctionWithName:mtl_func_name];

    NSError* error = nil;
    id<MTLComputePipelineState> compute_pipeline_state =
        [device newComputePipelineStateWithFunction:mad_throughput error:&error];
    ...
    // We need to populate src buffers using the CPU and then use them on the GPU, hence the managed
    // storage mode.
    ...
    // We need to read the dst buffer on the CPU to verify the results, hence the managed storage
    // mode.
    ...
    const MTLSize max_threads_per_group = [device maxThreadsPerThreadgroup];
    ...
    // [mvk-error] VK_ERROR_DEVICE_LOST: Command buffer 0x7fd63a80a600 "vkQueueSubmit CommandBuffer
    // on Queue 0-0" execution failed (code 2): Caused GPU Timeout Error (IOAF code 2)
    // mad_throughput_main.cc:170: check error: INTERNAL: VK_ERROR_DEVICE_LOST
